...
class Dataset:
    ...
    def standardize(self, cols):
        for col in cols:
            ...
            self.data[:, col] = data
            print(f"Standardized {self.headers[col]}")
            print(f"First row: {self.data[0, col]}")
...
def load_data():
    ...
    data.standardize(data.header_to_cols(["Dependents", "ApplicantIncome", "CoapplicantIncome", "LoanAmount", "Loan_Amount_Term"]))
    data.ohe(data.header_to_cols(["Gender", "Education", "Property_Area"]))
    data.mask_yes_no(data.header_to_cols(["Married", "Self_Employed", "Loan_Status"]))
    data.finalize(data.header_to_cols(["Loan_Status"])[0])
    x = data.get_data()
    y = data.get_output()
